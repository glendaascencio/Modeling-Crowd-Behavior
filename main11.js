...
Player.prototype.action = function() {
    ...
    if(roomIsSafe){
        ...
        if(
          ...
          (this.center.x >= rightbound || this.center.x <= leftbound || this.center.y >= lowbound || this.center.y <= upbound) 
        ){
        this.delete();
    }
...
var createDoors = function(){
  var a = new Door("left", 400, 200);
  var b = new Door("right", 400, 200);
...
function moveAll(){
    ...
    if(!done && mingling){
        enterBuilding();
        d3.select("#p5").attr("fill","orange");
    }

    // this loop should run after danger occurs and people want to leave.
    if(!done && !mingling && numEntered === numPlayers){
        exitBuiling();
        d3.select("#p5").attr("fill","orange");
    }
    else return done;
