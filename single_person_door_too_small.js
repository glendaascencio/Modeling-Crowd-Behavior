...
Player.prototype.lookAtExit = function(){
    ...
    console.log("lookatExit called by " + this.id + " and new xvel = " + x_comp + " new yvel = " + y_comp);
    return {
        xvel: x_comp,
        yvel: y_comp,
    }
...
Player.prototype.action = function() {
    ...
    if(roomIsSafe){ // bounce off walls if no danger in room
        ...
        if(this.center.x >= rightbound || this.center.x <= leftbound || this.center.y >= lowbound || this.center.y <= upbound){ 
            ...
            if((this.myExit && this.bounds.top >= this.myExit.bounds.top && this.bounds.bottom <= this.myExit.bounds.bottom)){
                console.log("fits");
...
function findCollisions(){
    ...
    for (pair in unique_pairs){
        ...
        if(existsCollision && roomIsSafe){
            ...
            if(same_xvel || same_yvel){ // faster one will slow down
                console.log(personInBack.id + " with a speed of " + personInBack.walking_speed + " is copying the speed of " + personInFront.id + " with a speed of " + personInFront.walking_speed);
                personInBack.walking_speed = personInFront.xvel;
                personInBack.hasMoved = true;
            }
            else{ // if going opposite directions they both bounce off
                console.log("going in opposite directions. r1 = " + r1.xvel + " r2 = " + r2.xvel );
                r1.xvel = r1.xvel * -1;
                r2.xvel = r2.xvel * -1;
                r1.yvel = r1.yvel * -1;
                r2.yvel = r2.yvel * -1;
...
var createDoors = function(){
  // door with padding
  // var a = new Door("right", 200, player_h + 10);

  //door without padding
  var a = new Door("right", 200, player_h);

  // var b = new Door("right", 600, 100);
